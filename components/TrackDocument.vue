<template>
  <div class="panel" :class="{'goblue': milestone ? isDone : true  }" v-tooltip="tooltipMessage">
    <div class="panel-body">
      ...
      <h2 class="text-uppercase">{{ milestone ? milestone.responseType.name : 'Report Submitted' }}</h2>
    </div>
  </div>
</template>
...
<script>
export default {
  props: ["milestone", "report"],
  computed: {
    isDone() {
      return this.milestone.resolvedAt !== null && this.milestone.confirmed;
    },
    tooltipMessage() {
      if (!this.milestone) {
