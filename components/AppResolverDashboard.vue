<template>
  <section>
    ...
    <div class="col-md-5">
      <div class="text-3xl">{{ $store.state.user.areas[0].name }}</div>
      <area-map :area="$store.state.user.areas[0]" :reports="reports"></area-map>
    </div>
  </section>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.bounds = L.latLngBounds(
      L.latLng(
        this.$store.state.user.areas[0].location.coordinates[0][1],
        this.$store.state.user.areas[0].location.coordinates[0][0]
      ),
      L.latLng(
        this.$store.state.user.areas[0].location.coordinates[1][1],
        this.$store.state.user.areas[0].location.coordinates[1][0]
      )
    );
