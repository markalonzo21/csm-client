<template>
  <div class="container p-2" style="height: 80vh;">
    ...
    <h3 class="mb-1">Assigned To:
      <template v-if="report.assignedTo">{{ report.assignedTo.firstName }} {{ report.assignedTo.lastName }}</template>
      <template v-else>
        <button @click.prevent="showAssignModal" class="underline cursor-pointer text-blue">Assign Respondent</button>
      </template>
    </h3>
    ...
    <modal v-model="isAssignModalVisible" :class="{ 'pointer-events-none': loadingAssignRespondent }">
      <select required v-model="selectedRespondent" class="form-control">
        <option :value="null">Select Respondent</option>
        <option v-for="respondent in availableRespondents" :value="respondent._id">
          {{  respondent.firstName }} {{ respondent.lastName }}
        </option>
      </select>
      <div slot="footer">
        <button @click.prevent="assignRespondent" class="btn btn-primary" :disabled="!selectedRespondent">Assign</button>
      </div>
    </modal>
  ...
  export default {
    ...
    methods: {
      ...
      showAssignModal() {
        ...
        this.$axios.$get(`/admin/available-respondents?type=${this.report.reportType._id}`).then(response => {
          this.availableRespondents = response.data
        })
      },
      assignRespondent() {
        this.loadingAssignRespondent = true
        this.$axios.$post(`admin/assign-respondent`, {
